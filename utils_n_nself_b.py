#this script, we create the part for the beta utility version.
...
def get_matrix(triples, entity, rel, time, time_int, ent_tem_time_count,\
                ent_tem_int_time_count, tem_valid_set, tem_int_valid_set,\
                      time_dict_i=False, time_int_dict_i=False):
    ...
    for i in range(len(ent_tem_time_count)):
        ...
        for j in tem_times_rec:
            ...
            tf = np.log(1 + time_j)
            #time_link[i][j] = tf
            time_link[i][time_dict_i[j]] = tf if j in tem_valid_set else 0.0
    ...
    return \
        adj_matrix, adj_features, rel_features, time_features, time_int_features, \
            radj, time_dict_i, time_int_dict_i


def load_data_i(lang, snapshot, train_ratio=0.3, \
                unsup=False, dev_file=False, time_dict_i=False, time_int_dict_i=False):
        ...
        get_matrix(triples1 + triples2, \
                   entity1.union(entity2), rel1.union(rel2), time1.union(time2), time_int1.union(time_int2),\
                      ent_tem_time_count, ent_tem_int_time_count, tem_valid_set, tem_int_valid_set,\
                        time_dict_i=time_dict_i, time_int_dict_i=time_int_dict_i)

    return np.array(train_pair), np.array(dev_pair), \
    adj_matrix, adj_features, rel_features, time_features, \
        time_int_features, radj, time_dict_i, time_int_dict_i,\
