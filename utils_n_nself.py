...
def load_triples(file_name):
    ...
    for line in open(file_name, 'r'):
        para = line.split()
        if len(para) == 5:
            head, r, tail, ts, te = [int(eval(item)) for item in para]
            # if ts != 0 and te == 0:
            #     te = ts
            # if te != 0 and ts == 0:
            #     ts = te
...
def get_matrix(triples, entity, rel, time, time_int, ent_tem_time_count,\
                ent_tem_int_time_count, tem_valid_set, tem_int_valid_set, time_int_dict_i=False):
    ent_size = max(entity) + 1 + 1
    #here, for the relation, we add with one extra one.
    rel_size = max(rel) + 1 + 1
    #time_size = max(time) + 1
    time_size = 979
    if time_int_dict_i is False:
        time_int_size = len(time_int)
    else:
        time_int_size = len(time_int_dict_i)
    ...
    #     adj_matrix[i, i] = 1
    adj_matrix[ent_size - 1, ent_size - 1] = 1
    ...
    adj_features[max(entity)+1, max(entity)+1] = 1
...
def load_data_(lang, snapshot, train_ratio=0.3, unsup=False, dev_file=False):
    entity_y, rel_y, triples_y, time_int_y, time_y = load_triples(lang + 'triples_nt1')
    entity_w, rel_w, triples_w, time_int_w, time_w = load_triples(lang + 'triples_nt2')
    entity1, rel1, triples1, time_int1, time1 = load_triples(lang + snapshot + 'triples_1')
    entity2, rel2, triples2, time_int2, time2 = load_triples(lang + snapshot + 'triples_2')
    entity1 |= entity_y; rel1 |= rel_y; triples1 += triples_y; time_int1 |= time_int_y; time1 |= time_y
    entity2 |= entity_w; rel2 |= rel_w; triples2 += triples_w; time_int2 |= time_int_w; time2 |= time_w
    #here, we would like to first get triples
    ...
    if not dev_file:
        train_pair = []
        dev_pair = load_alignment_pair(lang + 'ref_pairs')
        all_pair_ = train_pair + dev_pair
        all_pair = []
        #select pairs.
        for pair in all_pair_:
            if pair[0] in entity1:
...
def load_data_i(lang, snapshot, train_ratio=0.3, unsup=False, dev_file=False, time_int_dict_i=False):
    entity_y, rel_y, triples_y, time_int_y, time_y = load_triples(lang + 'triples_nt1')
    entity_w, rel_w, triples_w, time_int_w, time_w = load_triples(lang + 'triples_nt2')
    entity1, rel1, triples1, time_int1, time1 = load_triples(lang + snapshot + 'triples_1')
    entity2, rel2, triples2, time_int2, time2 = load_triples(lang + snapshot + 'triples_2')

    entity_1a, rel1a, triples_1a, time_int1a, time1a = load_triples(lang + snapshot + 'triples_1a')
    entity_2a, rel2a, triples_2a, time_int2a, time2a = load_triples(lang + snapshot + 'triples_2a')
    ...
    adj_matrix, adj_features, rel_features, time_features, time_int_features, radj, time_int_dict_i= \
        get_matrix(triples1 + triples2, \
                   entity1.union(entity2), rel1.union(rel2), time1.union(time2), time_int1.union(time_int2),\
                      ent_tem_time_count, ent_tem_int_time_count, tem_valid_set, tem_int_valid_set,\
                        time_int_dict_i=time_int_dict_i)

    return np.array(train_pair), np.array(dev_pair), \
    adj_matrix, adj_features, rel_features, time_features, \
        time_int_features, radj, time_int_dict_i,\
        triples1, triples2, entity_1a, entity_2a

def load_data_ib(lang, snapshot, train_ratio=0.3, unsup=False, dev_file=False, time_int_dict_i=False):
    entity_y, rel_y, triples_y, time_int_y, time_y = load_triples(lang + 'triples_1n')
    entity_w, rel_w, triples_w, time_int_w, time_w = load_triples(lang + 'triples_2n')
    entity1, rel1, triples1, time_int1, time1 = load_triples(lang + snapshot + 'triples_1')
    entity2, rel2, triples2, time_int2, time2 = load_triples(lang + snapshot + 'triples_2')
