...
def cal_sims_(test_pair, feature, right=None):
    if right is None:
        # test_pair should be a LongTensor (indices)
        feature_a = feature[test_pair[:, 0]]
        feature_b = feature[test_pair[:, 1]]
    else:
        feature_a = feature[:right]
        feature_b = feature[right:]

    fb = feature_b.transpose(0, 1)  # Transpose: shape (dim, N)
    return torch.matmul(feature_a, fb)  # Shape: (M, dim) × (dim, N) → (M, N)

def sinkhorn_(sims, eps=1e-6):

    sims = torch.exp(sims * 50)

    for _ in range(10):
        sims = sims / (sims.sum(dim=1, keepdim=True) + eps)
        sims = sims / (sims.sum(dim=0, keepdim=True) + eps)
