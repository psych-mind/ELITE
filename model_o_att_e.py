...
class icl_loss(nn.Module):
    ...
    def forward(self, emb, train_links, neg_l=None, neg_r=None, norm=True):
        ...
        labels = F.one_hot(torch.arange(start=0, end=batch_size, \
                                        dtype=torch.int64), num_classes=num_classes).float()
        labels = labels.cuda()
        if neg_l is not None:
            labels_2 = F.one_hot(torch.arange(start=0, end=batch_size, \
                                              dtype=torch.int64), num_classes=num_classes_2).float()
...
class OverAll(nn.Module):
    def __init__(self, node_size, node_hidden, time_hidden,
                 ...
                 rel_matrix, rel_val,
                 ent_matrix, ent_val,
                 time_matrix, time_val, out_dim, interval_index,
    ...
    def forward(self, inputs):
        ...
        opt3 = [self.time_emb, adj_input, self.time_int_emb[self.int_index]]
        ...
        torch.cat([self.ent_emb[-1], self.ent_emb[-1], self.ent_emb[-1],\
                   ...
                   self.time_int_emb[2547], self.time_int_emb[2547], self.time_int_emb[2547]])
        ...
        # ref_emb_it = \
        #     torch.cat([self.time_int_emb[2547], self.time_int_emb[2547], self.time_int_emb[2547]])
        ...
        if self.return_loss:
            if self.add_batch:
                loss2 = self.criterion_cl_joint(out_feature, batch_a)
                loss = self.criterion_cl_joint(out_feature, batch)
                loss = loss + self.beta * loss2
            else:
                loss = self.criterion_cl_joint(out_feature, batch)
