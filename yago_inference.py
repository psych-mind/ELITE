...
def custom_top_k_gpu(x_s, x_t, batch_size=1000):
    ...
    for start in range(0, x_s.shape[0], batch_size):
        ...
        distances, indices = index_gpu.search(x_s[start:end], 5)
...
def greedy_unique_set_conf(sim, entity_wiki_n):
    a, b = sim.shape
    forbidden = set()
    selected = set()

    # process high-confidence rows first
    row_order = np.argsort(np.max(sim, axis=1))[::-1]
    sorted_idx = np.argsort(sim, axis=1)[:, ::-1]

    for i in row_order:
        for idx in sorted_idx[i]:
            if idx not in forbidden:
                selected.add(entity_wiki_n[idx])
                forbidden.add(idx)
                break
...
# with open('/home/jiayun/Desktop/temp_evo_result_refine/model/evolve_model_test/dev_pair_list.pkl', 'rb') \
#     as f:
#     dev_pair_list = pickle.load(f)
...
#then we form with the evaluation phase.
for j, snap_shot in enumerate(snap_shots):
    ...
    time_int_val, node_size, rel_size, time_size, time_int_size, triple_size \
        ...
        with torch.no_grad():
            ...
            sim = cal_sims(dev_pair_list[j], output)
            score_sr = sinkhorn(sim)
            hits_, acc, total_mean, wrong_index_0, wrong_rank_0, wrong_rank_index_0,\
                  wrong_scores_0, rank_0\
    ...
    time_int_val, node_size, rel_size, time_size, time_int_size, triple_size \
        ...
        #then we check with the part of the pre-trained result and do the combination
        #afterwards.
    #     model_o.load_state_dict(\
    # torch.load\
    # (f'/home/jiayun/Desktop/temp_evo_result_refine/model/evolve_model_test_s/50/0.8/model_refine_{snap_shot[:-1]}.pth'))
        model_o.load_state_dict\
        ...
        # selected_wiki_ent = greedy_unique_set_conf(score_s, entity_wiki_n)
        del sim
        del score_s
        gc.collect()
...
#         print('check coverage, combined, eovlving, extended')
#         print(check_cov(entity_1a, entity_wiki, all_pair_dic), check_cov(entity_1a, entity_2a, all_pair_dic),\
# check_cov(entity_start, entity_wiki, all_pair_dic))
        ...
        # all_indices_yago = custom_top_k_gpu(output[np.array(list(entity_2has))], output[dev_pair[:, 0]])
        # #update the initilized entity.
        # yago_real_index_1 = index_mod(all_indices_yago, yago_index_i, 1)
        # yago_real_set = set(list(yago_real_index_1))

        # entity_1has_ = entity_1has | yago_real_set
        # print(len(entity_1has_))
        ...
        # rec_i, rec_o = split_score_dic(rank_com_dic_sp, index_record_score_)
        # print(gen_acc_m(rec_i, all_pair_dic))
        # print(gen_acc_m(rec_o, all_pair_dic))
        # print(gen_acc_m(rank_com_dic_sp, all_pair_dic))
...
def refine_dict_pw(comb_p, comb_c, index_score, alpha, thres=0.01):
    """
    ...
    for index, score_dic in comb_c.items():
        if index not in comb_p:
            ...
            if index in index_score:
                ...
                if b - a > thres:
                    # weighted combine, favor comb_p
                    #alpha = 0.6
                    beta = 1 - alpha

                    keys_1 = index_score[index]
                    keys_2 = comb_p[index]

                    combined = {}
                    inner_keys = set(keys_1.keys()) | set(keys_2.keys())

                    for k in inner_keys:
                        if k in keys_1 and k in keys_2:
                            combined[k] = beta * keys_1[k] + alpha * keys_2[k]
                        elif k in keys_1:
                            combined[k] = keys_1[k]
                        else:
                            combined[k] = keys_2[k]
...
for value in test_list:
    ...
    for i in range(9):
        #target_p = rec_p[i]
        if i == 0:
            target_p = rec_score_list[0]
            target_comb = combine_ranks_multi(rec_score_list[: 2])
        else:
            target_p = rank_dic_update
            target_comb = combine_ranks_multi([rank_dic_update, rec_score_list[i + 1]])
        rank_dic_update = refine_dict_pw(target_p, target_comb, \
                                         rec_score_list[i + 1], 0.55, value)
